...
uses
  wurfl, StrUtils, SysUtils, Classes, eLibHashList, uWURFL;
...
function TPHPExporter.getPHPID(wurflID: integer;
  var phpHID, phpFID, phpGID, phpPath: string): boolean;
var
  def: TPHPDef;
  fid: integer;
  did: integer;
...
procedure TPHPExporter.exportRegistryPart(part: string; S: TList; iFrom, iTo: integer;
  append: boolean);
var
  i: integer;
  f: textfile;
...
begin
  ...
  for i:= iTo downto iFrom do begin
    ...
    if (h2.dev.Fall_back <> 'root') then begin
      ...
      writeln(f, Format('array("%s","%s","%s",%s),', [encode(hi.userAgent), phpHID, phpPath,
        BoolToStr(hi.endSearch)]));
    end;
  end;
  if (not append) then begin
    writeln(f, ');');
...
begin
  ...
  for i:= 0 to index.Count - 1 do begin
    ...
    writeln(f, Format('array("%s","%s"),', [encode(hi.userAgent), Format(PHP_REGFILE,
      [IntToStr(i)])]));
  end;
  writeln(f, ');');
  writeln(f, PHP_END);
